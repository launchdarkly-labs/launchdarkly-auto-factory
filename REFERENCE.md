# LaunchDarkly AutoFactory reference implementation

Detailed setup for an opinionated software factory built on LaunchDarkly Release Primitives.
A chain of LaunchDarkly-defined AI agents turns a plain pull request into a feature-flagged,
metric-instrumented, tested change, and a release orchestrator turns the eventual deploy
into a guarded rollout that monitors itself.

The reference implementation is shared with design partners. Phase 1 and Phase 2 both run
end to end against a live demo repo.
...
| `bootstrap/cursor-automation/` | Phase 1 front end #3 (native Cursor automation): a drop-in `.cursor/` rule + command + MCP config; runs in Cursor's own agent (local experiment) |
...
| **Native Cursor automation:** [`bootstrap/cursor-automation`](bootstrap/cursor-automation/) | the `/autofactory` command in Cursor | edits left in your working tree | Cursor's own models (no API key) | local experiment; cloud (auto, PR-based) is a later phase |
...
run warns in the Actions log and the PR summary comment, then points you back at
...
registry, no tool. Failures degrade to warnings, never failed runs.
