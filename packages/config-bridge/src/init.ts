...
export async function init(args: string[]): Promise<void> {
  ...
  console.log(
    `  ✓ provisioned: ${r.configsCreated.length} config(s), ${r.graphsCreated.length} graph(s), ${r.flagsCreated.length} flag(s), ${r.toolsCreated.length} tool(s) created` +
      ` (${r.configsExisting.length + r.graphsExisting.length + r.flagsExisting.length + r.toolsExisting.length} already existed)`,
  );
