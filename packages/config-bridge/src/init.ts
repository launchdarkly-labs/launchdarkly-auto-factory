...
function openSetupPr(appRepo: string, files: RenderedFile[], pr?: { title: string; body: string }): string | undefined {
  ...
  const prTitle = pr?.title ?? "Set up LaunchDarkly AutoFactory";
  const prBody =
    pr?.body ??
    "Adds the AutoFactory Phase 1 workflow (runs the agent chain on every PR) and the LaunchDarkly code-references scan. Generated by `bridge init`; repo secrets and the LD_APP_PROJECT_KEY variable were set alongside this PR. Merge, then open any PR to see the chain run.";
  const prResult = gh([
  ...
  if (prResult.ok) {
    return (JSON.parse(prResult.stdout) as { html_url?: string }).html_url;
  }
  ...
  throw new InitError(`Could not open the setup PR in ${appRepo}: ${prResult.stderr || prResult.stdout}`);
}
...
// --- Copilot cloud agent front end ----------------------------------------------

/**
 * Opens a setup PR adding the custom agent + copilot-setup-steps, then prints
 * the settings that have no gh/API support yet (Agents secrets/variables and
 * the firewall allowlist live in the repo Settings UI only).
 */
function setupCopilotFrontEnd(appRepo: string, factoryProjectKey: string, appProjectKey: string): void {
  if (!ghAvailable()) throw new InitError("GitHub CLI (gh) not found — install https://cli.github.com and run `gh auth login`, then re-run init");
  if (!ghAuthenticated()) throw new InitError("gh is not authenticated — run `gh auth login`, then re-run init");
  if (!ghJson([`repos/${appRepo}`])) throw new InitError(`App repo '${appRepo}' not reachable via gh — check the owner/name and your access`);

  console.log(`\nConfiguring Copilot cloud agent front end for ${appRepo}…`);

  const files: RenderedFile[] = [
    { path: ".github/agents/autofactory.agent.md", content: readFileSync("bootstrap/copilot/agents/autofactory.agent.md", "utf8") },
    { path: ".github/workflows/copilot-setup-steps.yml", content: readFileSync("bootstrap/copilot/copilot-setup-steps.yml", "utf8") },
  ];
  const url = openSetupPr(appRepo, files, {
    title: "Set up LaunchDarkly AutoFactory (Copilot cloud agent)",
    body:
      "Adds the `autofactory` custom agent and the `copilot-setup-steps.yml` that pre-installs the AutoFactory tooling in Copilot cloud agent's environment. " +
      "Generated by `bridge init`. Merge (both files must be on the default branch), complete the Settings checklist from INSTALL-COPILOT.md " +
      "(Agents secrets/variables + Copilot Internet-access allowlist), then assign an issue to Copilot with the `autofactory` agent.",
  });
  console.log(`  ✓ setup PR ready: ${url}`);

  // Cloud agent reads a distinct secrets store ("Agents") that gh cannot set yet.
  console.log("\n  Manual settings in the app repo (Settings UI — no gh/API support for these yet):");
  console.log("  1. Settings → Secrets and variables → Agents (NOT Actions):");
  console.log("       secrets   LD_SDK_KEY, LD_API_KEY, ANTHROPIC_API_KEY   (values are in this repo's .env)");
  console.log(`       variables LD_PROJECT_KEY=${factoryProjectKey}, LD_APP_PROJECT_KEY=${appProjectKey}`);
  console.log("       optional  AUTOFACTORY_MAINTAINER_EMAIL=<your LD member email> (flag maintainer attribution)");
  console.log("       never add APPROVAL_MODE or RISK_THRESHOLD here — they override the LD approval flags");
  console.log("  2. Settings → Copilot → Internet access: add custom domains 'launchdarkly.com' and 'api.anthropic.com'");
  console.log("  3. Copilot cloud agent enabled on the repo (org policy for Business/Enterprise) + healthy Actions billing");
  console.log("  Full guide: INSTALL-COPILOT.md");
}
...
export async function init(args: string[]): Promise<void> {
  ...
  if (!frontEnd) {
    const answer = (await ask("Front end — [g]itHub Action (default), [c]laude Code skill, code[x] skill, c[u]rsor automation, co[p]ilot cloud agent, or [n]one", "g")).toLowerCase();
  ...
  if (frontEnd === "github-action") {
    ...
    console.log("\nDone.");
  } else if (frontEnd === "copilot") {
    const appRepo = argValue(args, "app-repo") ?? process.env.AUTOFACTORY_APP_REPO ?? (await ask("App repo on GitHub (owner/name)"));
    if (!appRepo || !appRepo.includes("/")) throw new InitError("An app repo is required for the Copilot front end (--app-repo owner/name)");
    setupCopilotFrontEnd(appRepo, factoryKey, appKey);
    writeFileSync(".env", upsertEnv(readFileSync(".env", "utf8"), { AUTOFACTORY_APP_REPO: appRepo }));
    console.log("\nDone. Merge the setup PR, finish the Settings checklist above, then assign an issue to Copilot with the `autofactory` agent.");
