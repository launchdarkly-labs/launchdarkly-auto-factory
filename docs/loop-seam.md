...
|---|---|---|
| `held` | intent said hold/manual, a future `notBefore`, segments recorded-not-executed, a target the flag **has no variation for**, or a target off the vN lineage | **ledger re-checks on any later deploy**, re-reading the manifest at the current SHA — so a human's fix takes effect. Re-POST to retry sooner |
...
| `error`, `triggerRelease` threw | LD 5xx or a network failure mid-write | **ledger**, guarded by the terminal-status check below. And it **claims the flag's action slot**, because a throw is not "did not write" but "we do not know": `startRelease` awaits the response *after* the patch is applied, so a sibling manifest must not release a different variation of that flag in the same notification. That claim costs the deferred sibling a **delay** rather than its release only while no surviving throw is *both* deterministic *and* per-manifest — see the row below, which is where that property was previously asserted falsely |
| `held`, LaunchDarkly **rejected** a patch on CONTENT grounds | an allowlisted client error on either patch Beacon sends: LD answered, and its answer is that the patch did not apply | **ledger** (`held` is not final), reported with LaunchDarkly's own message. The dividing line is NOT pre-write vs post-write, which is what the row above used to claim: a throw driven by manifest **content** is deterministic too. `startRelease`'s instruction body carries `releasePlan.stages`, `metricKeys`, `metricGroupKeys` and `randomizationUnit` straight through, and **neither `write_manifest` nor Beacon validated any of them** — so a 100% guarded stage (LD caps guarded stages at 50%, quoted live in `trigger.ts`) was a permanent 400 which `targetRank` ran **first**: the releasable sibling was starved on every deploy and told "another manifest released this flag", which had not happened. **Both** patches are now classified — the release-start one and the `prerequisites` one, whose `addPrerequisite` instructions come from `releaseIntent` and where a **circular prerequisite** is the reachable 400 (`sandboxTools` checks only key syntax; `normalizePrerequisites` accepts any valid key). "Nothing was written" holds for these multi-instruction patches because LD documents that *"semantic patches are not applied partially; either all of the instructions are applied or none of them are"* — established, not assumed. Classified on `LdApiError.status`, never on message text, and it is an **ALLOWLIST** (`{400, 422}`) rather than the denylist it was. Two errors, in opposite directions, and the second is the instructive one: the first allowlist was `{400}`, derived by asking "what does this ENDPOINT document?" — but LaunchDarkly's error table is **API-WIDE**, not per-endpoint, and the row that describes a patch body is **422** ("the update description can not be understood… Ensure that the request body is correct for the type of patch you are using, either JSON patch or semantic patch"). So the allowlist is `{400, 422}`, and excluding 422 restored the very starvation the allowlist was written to remove. The question was wrong, not the answer. The denylist meanwhile treated three statuses as content when they are not — **409** is transient (LD's remediation is "Retry your request") and misclassifying it **changed production behaviour**, because `held` leaves the action slot open and a sibling wanting an *earlier* variation rolled out spuriously; **405** "approval is required" is per-**environment** (standard enterprise config on Beacon's own target); **404** "invalid resource identifier" is the flag or the environment. 429/408 also keep throwing (a spent rate-limit budget is transient; a timed-out request may have been processed — and 408 is nowhere in LD's v2 spec, so it is a proxy's), as do 401/403 — **per-flag or per-environment at worst, never per-manifest**, since LD's role resource specifiers are globbed and environment-scoped (`proj/*:env/*:flag/ops_*`) rather than "global" as this doc used to say; the conclusion survives because per-flag starves nobody and LD has no separate role action for guarded vs progressive. An **unknown 4xx keeps throwing** rather than being called a manifest defect. `write_manifest` now also checks `stages` at authoring time — defence in depth, not the guarantee, since `.release-flags/` is hand-editable and the other three fields still arrive unchecked. **Residual:** whatever LD rejects with a non-client error, plus any 4xx it does not document here |
...
thing the deleted branch was right about. That repoint takes one further read first: `completed` is
a fact about a **moment in the past** while the destination is the parent's **live** fallthrough, so
it fires only while nothing is running on the flag, matching the `!active` fence `monitor.ts`
already had. See the next section.

And the destination itself is now bounded, inside `repoint.ts` rather than at any caller's gate, so
all three callers get it: **a child is never repointed backwards.** "Live" includes states a human
deliberately put the flag into, and `trigger.ts` recommends serving an earlier variation directly as
the way to roll back — after which the old release still reads `completed`, so every gate above is
satisfied and the destination is `control`. A child pinned behind an unmet prerequisite is **dark**;
repointing it to what the parent now serves *meets* that prerequisite and takes it live for 100% of
traffic with no rollout, **as a consequence of a rollback**. So a child pinned to `vN` is skipped
when the destination ranks lower or has no lineage index at all, and `control` → `v1` / `v1` → `v2`
are untouched. This needed no datum the module lacked, which is why it is fixed while the *forward*
mid-rollout hazard is not: `pinned` and `serving` are both already in hand, whereas aiming at the
release's real target would need a field `AutomatedRelease` does not carry.
...
- **`reEvaluate`'s `completed` branch** treated "some release of this flag finished" as "this
  ...
  slot deferral, and (final, so genuinely the last chance ever) scope `skipped` — never reached the
  surviving repoint. **Not manifest-absent**, which was measured rather than assumed: that check
  now lives in the pending pass, *ahead* of `reEvaluate`, so a withdrawn manifest never reaches the
  repoint and — being final — never will. Deliberately so. A fresh read that finds no manifest
  leaves nothing to aim at but the remembered `entry.flagKey`, and feeding remembered content into
  a guard is exactly the content-as-identity mistake these rounds took out of the ledger; a missed
  repoint is recoverable by any later deploy carrying any manifest for that flag, and by release
  monitoring, while a repoint aimed at the wrong flag is not.
