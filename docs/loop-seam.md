...
| `held`, LaunchDarkly **rejected** the release instruction | a 4xx on the release-start patch: LD answered, and its answer is that the patch did not apply | **ledger** (`held` is not final), reported with LaunchDarkly's own message. The dividing line is NOT pre-write vs post-write, which is what the row above used to claim: a throw driven by manifest **content** is deterministic too. `startRelease`'s instruction body carries `releasePlan.stages`, `metricKeys`, `metricGroupKeys` and `randomizationUnit` straight through, and **neither `write_manifest` nor Beacon validated any of them** — so a 100% guarded stage (LD caps guarded stages at 50%, quoted live in `trigger.ts`) was a permanent 400 which `targetRank` ran **first**: the releasable sibling was starved on every deploy and told "another manifest released this flag", which had not happened. Classified on `LdApiError.status`, never on message text, and 429/408/401/403 deliberately keep throwing (a spent rate-limit budget and a timed-out request are transient; credentials are global, so they starve nobody). `write_manifest` now also checks `stages` at authoring time — defence in depth, not the guarantee, since `.release-flags/` is hand-editable and the other three fields still arrive unchecked. **Residual:** whatever LD rejects with a non-client error, plus one known gap — only the *release-start* patch is classified, so a 4xx on the `prerequisites` release's `addPrerequisite` patch (built from `releaseIntent`) is still deterministic, per-manifest, and still claims the slot |
